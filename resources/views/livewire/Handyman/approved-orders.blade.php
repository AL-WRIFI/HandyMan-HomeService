...
{{--
                                    ...
                                    <div class="row ">
                                        @if($order->dateTimeService < \Carbon\Carbon::now())
                                        <button class="col-5 col-md-3 p-0  btn btn-success btn-sm"
                                            wire:click="ongoingOrder({{$order->id}})">بدءالعمل</button>
                                        @else
                                        <a style="text-align-last: right">لايمكنك بدء العمل فبل موعد الزيارة</a>
                                        <button class="col-5 col-md-3 p-0  btn  btn-sm" style="background-color: #c9c8c8"> بدءالعمل </button>
                                        @endif
                                    </div>
                                    @else
                                    <div class="row ">
                                        <button class="col-5 col-md-3 p-0  btn btn-success btn-sm" 
                                            wire:click="completedOrder({{$order->id}})">انهاء العمل</button>
                                    </div>
                                    @endif
                                </div>
...
<script>
    // Get the timestamp value from the hidden input field
    var timestamp = document.getElementById('timestamp').value;

    // Get the current timestamp value
    var currentTimestamp = Math.floor(Date.now() / 1000);

    // Compare the current timestamp with the specified timestamp
